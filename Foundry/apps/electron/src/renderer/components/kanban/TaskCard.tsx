    ...
    <div
      className={`rounded-lg border bg-white p-3 dark:bg-zinc-900 ${
        isOverlay
          ...
          : `border-zinc-200 border-l-2 ${colors.border} cursor-grab active:cursor-grabbing`
      }`}
    >
...
export function SortableTaskCard({ task, onClick }: SortableProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={onClick}>
      <div className={`cursor-pointer rounded-lg border border-l-2 border-zinc-200 bg-white p-3 transition-colors hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700 ${colors.border}`}>
        <div className="flex items-center justify-between gap-2">
          <span className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] font-semibold ${colors.bg} ${colors.text}`}>
            {TASK_PRIORITY_LABELS[task.priority as keyof typeof TASK_PRIORITY_LABELS] ?? task.priority}
          </span>
